# Pack v14 production-readiness evidence

`gate-status.json` is the versioned promotion ledger for Pack v14. Every gate is one of `PASS`,
...
The v13 baseline must come from a separate clean worktree at `dd373e3`; never switch or rewrite the
...
v14-only manifest fields and records them as `unavailable` on v13.

```bash
export ANDROID_HOME=/opt/homebrew/share/android-commandlinetools
export BASELINE_DIR=/tmp/deal-studio-v13-baseline
export CANDIDATE_DIR=/Users/egavrin/Documents/deal-studio
git -C "$CANDIDATE_DIR" worktree add --detach "$BASELINE_DIR" dd373e3
...
cp "$CANDIDATE_DIR/app/src/androidTest/assets/pack-v14-benchmark-v1.json" \
  "$BASELINE_DIR/app/src/androidTest/assets/"
cp "$CANDIDATE_DIR/app/src/androidTest/kotlin/com/offlineassistant/app/generatedapp/CanonicalPackV14GenerationDeviceTest.kt" \
...
The default is five repeats. Odd repetitions run v13 then v14; even repetitions run v14 then v13.
...
```bash
"$CANDIDATE_DIR/tooling/deal-ui-pack/benchmarks/v14/run-paired-device-benchmark.sh" \
  --serial "$ANDROID_SERIAL" \
  --output "$CANDIDATE_DIR/build/pack-v14-benchmark/paired-$(date +%Y%m%d-%H%M%S)" \
