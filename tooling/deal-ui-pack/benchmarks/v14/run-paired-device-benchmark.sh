...
TEST_CLASS="com.offlineassistant.app.generatedapp.CanonicalPackV14GenerationDeviceTest"
...
DATASET="$REPO_ROOT/app/src/androidTest/assets/pack-v14-benchmark-v1.json"
CASES=()
...
run_one() {
  ...
  if [[ "$variant" == "v13-baseline" ]]; then
    expected_pack_version="deal-studio-dealui-pack-v13"
  else
  ...
  if ! adb -s "$SERIAL" exec-out run-as "$PACKAGE_ID" \
      tar -C files/pack-v14-generation -cf - "$run_id" 2> "$destination/pull-error.txt" \
...
for ((repetition = 1; repetition <= REPEATS; repetition++)); do
  if ((repetition % 2 == 1)); then
    variants=("v13-baseline" "v14-candidate")
  else
    variants=("v14-candidate" "v13-baseline")
  fi
