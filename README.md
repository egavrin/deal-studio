...
- cancellable background Exa Agent research with a fixed result card;
- numbered, tappable and persisted source cards;
...
- `web_research`: Exa Agent runs asynchronously with `minimal` effort and a bounded
  summary/findings schema. The composer is released after the run starts, and its
  `ResearchCard` remains cancellable.
